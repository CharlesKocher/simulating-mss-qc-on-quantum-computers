...
from pauli import Pauli, label_to_pauli
...
def eval_hamiltonian(Q_program, hamiltonian, input_circuit, shots, device):
    ...
    if shots == 1:
        ...
        if type(hamiltonian) is not list:
            ...
            print('1')
            result = Q_program.execute(circuit, device, shots=shots,config={"data": ["quantum_state"]})
            print('2')
            quantum_state = result.get_data(circuit[0]).get('quantum_state')
            if quantum_state is None:
                quantum_state = result.get_data(circuit[0]).get('quantum_states')
                if len(quantum_state) > 0:
                    quantum_state = quantum_state[0]
                                                                           
                # Diagonal Hamiltonian represented by 1D array
                if (hamiltonian.shape[0] == 1 or np.shape(np.shape(np.array(hamiltonian))) == (1,)):
                    ...
                    energy = np.inner(np.conjugate(quantum_state), np.dot(hamiltonian, quantum_state))
                    ...
                    for p in hamiltonian:
                        ...
                        for p in hamiltonian:
                            ...
                            energy += p[0] * np.inner(np.conjugate(quantum_state_0),quantum_state_i)
                            i += 1
...
def trial_circuit_rxrz(n, m, theta, entangler_map, meas_string=None,
                       measurement=True):
    """Creates a QuantumCircuit object ocnsisting in layers of
        parametrized single-qubit X and Z rotations and CZ two-qubit gates
        
        Args:
        n (int) : number of qubits
        m (int) : depth of the circuit
        theta array[float] : angles that parametrize the X and Z rotations
        entangler_map : CZ connectivity, e.g. {0: [1], 1: [2]}
        meas_string (str) : measure a given Pauli operator at the end of the
        circuit
        measurement (bool) : whether to measure the qubit (register "q")
...
def trial_circuit_raff(n, m, theta, entangler_map):
    """Creates a quantum circuit that we threw together"""
    q = QuantumRegister("q", n)
    c = ClassicalRegister("c", n)
    trial_circuit = QuantumCircuit(q, c)
    trial_circuit.h(q)
